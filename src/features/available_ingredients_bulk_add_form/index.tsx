...
import type {
  AvailableIngredientsBulkAddFormProps,
  ShoppingCartAlreadyGotDraft,
} from "./types";
...
export function AvailableIngredientsBulkAddForm(
  props: AvailableIngredientsBulkAddFormProps,
) {
  const initialRows = Object.entries(
    structuredClone(props.ingredientsToAdd),
  ...
  const [newUnit, setNewUnit] = createSignal<Unit>({
    type: "builtin",
    unit: "grams",
  });
  ...
  const addPantryBatch = useMutation(
    api.pantry_exports.AvailableIngredientsBulkAdd,
  );
  ...
  const allocationFor = (row: IntakeRow): Measurement | undefined => {
    ...
    const already =
      plannedToday()?.shoppingCart.alreadyGot[name] ?? ZeroedMeasurement();
    if (Measurement_GTE(already, target)) return;
    return Measurement_Min(Measurement_Minus(target, already), {
      amount,
      unit: row.unit,
    });
  };
  ...
  const validateAmount = (index: number) => {
    ...
    setRows(
      index,
      "amountError",
      !rows[index].amount.trim() || !Number.isFinite(amount) || amount < 0
  ...
  const validateRows = () => {
    rows.forEach((_, index) => {
      normalizeRowName(index);
      validateAmount(index);
    });
    return (
      rows.length > 0 &&
      rows.every(
        (row) => !row.nameError && !row.amountError && !!row.name.trim(),
      )
    );
  };
  ...
  const openAddIngredient = () => {
    ...
    queueMicrotask(() => newNameInput.focus());
  };
  ...
  const addIngredient = () => {
    ...
    const nameError = !name
      ? "Enter an ingredient name."
      : rows.some((row) => row.name.trim().toLowerCase() === name)
        ? `${name} is already in this batch.`
        : "";
    const amountError =
      !newAmount().trim() || !Number.isFinite(amount) || amount < 0
        ? "Enter an amount of 0 or more."
        : "";
  ...
  const submitIngredients = async (event: SubmitEvent) => {
    ...
    if (!validateRows()) {
      queueMicrotask(() =>
        document
          .querySelector<HTMLElement>(".intake-row [aria-invalid='true']")
          ?.focus(),
      );
      return;
    }
    setSubmitError("");
    const ingredientsToAdd = rows.map((row) => ({
      name: row.name.trim().toLowerCase(),
      Measurement: { amount: Number(row.amount), unit: row.unit },
    }));
    try {
      ...
      for (const row of rows) {
        ...
        if (row.cartSelected && allocation)
          allocations[row.name.trim().toLowerCase()] = allocation;
      }
      ...
      <header class="page-heading">
        <div>
          ...
          <p class="supporting-copy">
            Review what came into the kitchen before updating the pantry.
          </p>
        </div>
      </header>
      <form onSubmit={submitIngredients} novalidate>
              ...
              <div class="intake-row">
                <label class="field ingredient-field">
                  ...
                  <input
                    ...
                    aria-describedby={
                      row.nameError ? `${row.id}-name-error` : undefined
                    }
                    oninput={debounce(
                      (event) => {
                        ...
                        if (event.target.value) {
                          setRows(index(), "name", event.target.value)
                        } else {
                          console.log("was gonna")
                        }
                      },
                      300
                    )}
                    onBlur={() => normalizeRowName(index())}
                  />
                  <Show when={row.nameError}>
                    <span class="field-error" id={`${row.id}-name-error`}>
                      {row.nameError}
                    </span>
                  </Show>
                </label>
                <label class="field">
                  ...
                  <input
                    ...
                    aria-describedby={
                      row.amountError ? `${row.id}-amount-error` : undefined
                    }
                    onInput={(event) =>
                      setRows(index(), "amount", event.currentTarget.value)
                    }
                    onBlur={() => validateAmount(index())}
                  />
                  <Show when={row.amountError}>
                    <span class="field-error" id={`${row.id}-amount-error`}>
                      {row.amountError}
                    </span>
                  </Show>
                </label>
                <label class="field">
                  ...
                  {function(){
                          ...
                          whenSelected={(unit) => {
                            ...
                            if (!allocationFor(rows[index()]))
                              setRows(index(), "cartSelected", false);
                          }}
                          />
                        }()}
                </label>
                ...
                <Show when={allocationFor(row)}>
                  {(allocation) => (
                    <label class="cart-match">
                      <input
                        type="checkbox"
                        checked={row.cartSelected}
                        onChange={(event) =>
                          setRows(
                            index(),
                            "cartSelected",
                      ...
                      />
                      <span>
                        Count up to <Amount measurement={allocation()} /> toward
                        today’s cart
                      </span>
                    </label>
                  )}
        ...
        <Show when={rows.length === 0}>
          <div class="empty-state">
            <h2>No ingredients in this batch.</h2>
            <p>Add an ingredient before updating the pantry.</p>
          </div>
        </Show>
        <Show when={isAddingIngredient()}>
          <div
            class="add-ingredient-panel"
            role="dialog"
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === "Escape") closeAddIngredient();
            }}
          >
            <div class="popover-head">
              ...
              <button
                class="icon-button"
                type="button"
                aria-label="Close add ingredient"
            ...
            <div class="add-ingredient-fields">
              <label class="field ingredient-field">
                ...
                <input
                  ...
                  aria-describedby={
                    newNameError() ? "new-ingredient-name-error" : undefined
                  }
                ...
                />
                <Show when={newNameError()}>
                  <span class="field-error" id="new-ingredient-name-error">
                    {newNameError()}
                  </span>
                </Show>
              </label>
              <label class="field">
                ...
                <input
                  ...
                  aria-describedby={
                    newAmountError() ? "new-ingredient-amount-error" : undefined
                  }
                  onInput={(event) => {
                ...
                />
                <Show when={newAmountError()}>
                  <span class="field-error" id="new-ingredient-amount-error">
                    {newAmountError()}
                  </span>
                </Show>
              </label>
              <label class="field">
                ...
                {function(){
                  const customUnits = useQuery(api.customUnit_exports.getCustomUnits, {associatedIngredient: newName()});
                  return <Select
        ...
        <div class="form-actions">
          ...
          <button
            class="button button-primary"
            type="submit"
            disabled={addPantryBatch.isLoading() || rows.length === 0}
          >
            {addPantryBatch.isLoading() ? "Adding to pantry…" : "Add to pantry"}
          </button>
        </div>
      </form>
    </main>
