...
- Every screen previously carried the static `<title>Captain Cook</title>`, failing WCAG 2.4.2 (Page Titled) and making history/tabs indistinguishable. Added reactive titles: `AppShell` sets `<Destination> — Captain Cook` from `location.pathname` (the same reactive source as the visible route label), the auth gate sets `Sign in — Captain Cook` whenever unauthenticated, and `NotFound` sets `Page not found — Captain Cook` on mount. Redirect passthroughs (`/`, `/sign-in`) fall back to plain `Captain Cook` so no "not found" title flashes mid-redirect.
