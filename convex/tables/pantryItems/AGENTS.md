...
- Pantry writes replace one stored measurement, creating the item when it is absent, or additively apply a batch intake.
...
- `updateAvailableIngredient` replaces the stored measurement for an existing ingredient and creates the owned item when the user sets a measurement for an ingredient the pantry does not hold yet; it returns the stored document.
- `table.ts` exports `pantryItemValidator` and `pantryItemDocumentValidator`; server functions reuse them instead of restating the pantry-item shape.
