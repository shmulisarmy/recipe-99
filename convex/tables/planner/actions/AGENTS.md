...
- Reach planner days through `../days`; a mutation that names a date uses `getOrCreatePlannerDay` so an unplanned date is created on the spot with one person and an empty shopping cart.
- `updateDayMultiplier` accepts the date and persisted people-eating multiplier, creating the day when the user sets a people count on a date they have not planned yet.
...
- Adding or moving a recipe, editing a cart, or pushing cart items forward to a date without a planner document creates that day with one person and an empty shopping cart.
- Pushing an ingredient the day never asked for is a no-op for that ingredient rather than a failure of the whole batch.
